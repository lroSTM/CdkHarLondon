...
int32_t BSP_MOTION_SENSOR_Fifo(uint32_t Instance, uint32_t Functions,uint32_t level)
{
	...
	else if (Functions != MOTION_ACCELERO)
	{
	...
	else
	{
		...
		ISM330DHCX_Object_t * pObj = (ISM330DHCX_Object_t *) Motion_Sensor_CompObj[0];
		ism330dhcx_fifo_xl_batch_set(&pObj->Ctx, ISM330DHCX_XL_BATCHED_AT_26Hz);
		ism330dhcx_fifo_watermark_set(&pObj->Ctx,level);
		int1_route.int1_ctrl.int1_fifo_th = 1;
		ism330dhcx_pin_int1_route_set(&pObj->Ctx, &int1_route);
		ism330dhcx_fifo_mode_set(&pObj->Ctx, ISM330DHCX_STREAM_MODE);
	}
...
int32_t ISM330DHCX_Get_Fifo(uint32_t Functions, uint32_t Level, int8_t* p_buffer)
{
	...
	ism330dhcx_read_reg(&pObj->Ctx, ISM330DHCX_FIFO_STATUS1, reg, 2);
	fifo_level = ((reg[1] & 0x03) << 8) + reg[0];

	if(((reg[1]) & 0x80) && (fifo_level >= Level))
	{
		ism330dhcx_read_reg(&pObj->Ctx, ISM330DHCX_FIFO_DATA_OUT_TAG,(int8_t*)p_buffer, Level * 7);
	}
