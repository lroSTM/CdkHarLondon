...
def get_pipeline(
    ...
    q_test_level_acc_threshold = ParameterFloat(
        name="q_test_level_acc_threshold",
        default_value=0.6,
    )
    ...
    tf2_estimator = TensorFlow(
        ...
        metric_definitions=[
            ...
            {'Name': 'test:accuracy', 'Regex': 'Test-level accuracy on test set : ([0-9\\.]+)'}
        ],
        environment={
    ...
    step_fail = FailStep(
        ...
        error_message=Join(on=" ", values=[
            "Execution failed due to test level acc <", q_test_level_acc_threshold,
        ])
    )
    ...
    cond_lte_test = ConditionGreaterThanOrEqualTo(
        left=JsonGet(
            step_name=step_eval.name,
            property_file=evaluation_report,
            json_path="multiclass_classification_metrics.test_acc.value",
        ),
        right=q_test_level_acc_threshold,
    )
